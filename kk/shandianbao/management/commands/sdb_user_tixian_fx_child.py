...
class Command(BaseCommand):
    ...
    def handle(self, phone, *args, **options):
        now = datetime.now()
        print "__sync sdb user tixian child __ fanxian"
        print now
        if phone == "all":
            objs = models.SDBUserRMB.objects.filter(fanxian_child_rmb__gte=MIN_RMB).filter(is_auto=True)
            tixian(objs)
        else:
...
def tixian(objs):
    for obj in objs:
        print "start:", obj.user, obj.fanxian_child_rmb
        user = obj.user
        user_rmb = obj.fanxian_child_rmb
        if user_rmb < MIN_RMB:
        ...
        # give user wx rmb
        desc = u"返现发放"
        res = wx_tixian(wx_user.openid, str(real_rmb), name, desc=desc)
