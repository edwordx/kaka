...
class Command(BaseCommand):
    ...
    def handle(self, phone, *args, **options):
        now = datetime.now()
        print "__sync sdb user child three tixian"
        print now
        if phone == "all":
...
def tixian(objs):
    for obj in objs:
        print "start:", obj.user, obj.child_three_rmb
        user = obj.user
        user_rmb = obj.child_three_rmb
        if user_rmb < MIN_RMB:
        ...
        tx = models.SDBTiXianOrder.objects.create(
            ...
            profit=profit,
            tax=tax,
            order_type="CHILD_THREE_RMB",
        )
        dbutils.sub_sdbuserrmb_child_three_rmb(user, tx.rmb)
